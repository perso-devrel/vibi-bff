...
class AutoSubtitleService(
     ...
     */
    fun submitRegenerate(srtFile: File, spec: com.vibi.bff.model.SubtitleRegenerateSpec): String {
        val jobId = "sub-${UUID.randomUUID()}"
        val dir = File(outputDir, jobId).apply { mkdirs() }
    ...
    private suspend fun runRegeneratePipeline(job: SubtitleJob, spec: com.vibi.bff.model.SubtitleRegenerateSpec) {
    ...
    private suspend fun runPipeline(job: SubtitleJob, sourceFile: File, spec: SubtitleSpec) {
        ...
        // 3) 진행 상황 폴링. 첫 polling 전 짧은 grace (3s) — Perso 가 등록 직후 progress
        // 0% 만 응답하는 윈도우 회피. 이후 PersoPolling 의 backoff 가 적당한 간격으로
        // 진행. 기존 15s 는 짧은 영상에선 idle 처럼 보여 사용자 혼동.
        job.status = "PROCESSING"
        delay(3_000L)
     ...
     */
    private suspend fun extractAudioForStt(input: File, output: File) {
        log.info("ffmpeg extract audio: input={} ({} bytes) output={}", input.name, input.length(), output.absolutePath)
        extractMp3(input, output, quality = 5, timeoutMinutes = 5, label = "ffmpeg audio extract")
